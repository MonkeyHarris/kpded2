...
{
    MMDB_data_pool_s *const pool = (MMDB_data_pool_s *)calloc(1, sizeof(MMDB_data_pool_s));
    if (!pool) {
        return NULL;
    }
    ...
    pool->size = size;
    pool->blocks[0] = (MMDB_entry_data_list_s*)calloc(pool->size, sizeof(MMDB_entry_data_list_s));
    if (!pool->blocks[0]) {
        data_pool_destroy(pool);
        return NULL;
    }
...
MMDB_entry_data_list_s *data_pool_alloc(MMDB_data_pool_s *const pool)
{
    ...
    pool->blocks[new_index] = (MMDB_entry_data_list_s*)calloc(new_size, sizeof(MMDB_entry_data_list_s));
